...
- Emit every runtime warning under a dedicated `autoread_dotenv.warnings.AutoreadDotenvWarning`
  category (a `UserWarning` subclass) instead of a bare `UserWarning`. Callers can now silence
  just this package -- e.g. `PYTHONWARNINGS=ignore::autoread_dotenv.warnings.AutoreadDotenvWarning`
  -- without muting unrelated `UserWarning`s. Documented under "Silencing warnings" in
  `docs/configuration.md`. Also fixes the module-reload order in `tests/conftest.py` so reloaded
  modules re-bind to the fresh category.
